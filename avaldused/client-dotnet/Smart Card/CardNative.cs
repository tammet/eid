...
namespace GemCard
{
	...
	public class CardNative : CardBase
	{
		...
		public void EstablishContext(SCOPE Scope)
		{
			...
			if (m_nLastError != 0)
			{
				string msg = "SCardEstablishContext error: " + new Win32Exception(m_nLastError).Message;
		...
		public void ReleaseContext()
		{
			if (m_hContext != 0)
			{
				...
				if (m_nLastError != 0)
				{
                    string msg = "SCardReleaseContext error: " + new Win32Exception(m_nLastError).Message;
					throw new Exception(msg);
				}
		...
		public override APDUResponse Transmit(APDUCommand ApduCmd)
		{
			...
			if (m_nLastError != 0)
			{
                string msg = "SCardTransmit error: " + new Win32Exception(m_nLastError).Message;
				throw new Exception(msg);
			}
        ...
        public override void BeginTransaction()
        {
            if (m_hCard != 0)
            {
                ...
                if (m_nLastError != 0)
                {
                    string msg = "SCardBeginTransaction error: " + new Win32Exception(m_nLastError).Message;
                    throw new Exception(msg);
                }
            }
        }
        ...
        public override byte[] GetAttribute(UInt32 AttribId)
        {
            ...
            if (m_nLastError == 0)
            {
                if (attrLen != 0)
                {
                    ...
                    if (m_nLastError != 0)
                    {
                        string msg = "SCardGetAttr error: " + new Win32Exception(m_nLastError).Message;
                        throw new Exception(msg);
                    }
                }
            }
